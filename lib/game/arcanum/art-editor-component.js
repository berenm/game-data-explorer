...
export default class ARTEditorView extends ImageFileEditorComponent {
  ...
  update(properties, children) {
    ...
    child.execSync(`convert -background transparent ${frameList} -set delay ${100 / this.parser.header.frame_rate} -set dispose background -page ${maxOffX + maxWidth}x${maxOffY + maxHeight} '${this.file.path}.gif'`)
