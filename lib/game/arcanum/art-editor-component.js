...
export default class ARTEditorView extends ImageFileEditorComponent {
  ...
  update(properties, children) {
    let frameList = `-dispose previous -delay ${this.parser.header.frame_rate}x1000`
    let maxWidth = 0
    let maxHeight = 0
    let maxOffX = 0
    let maxOffY = 0
    ...
    child.execSync(`convert -size ${maxOffX + maxWidth}x${maxOffY + maxHeight} xc:none ${frameList} '${this.file.path}.gif'`)
    super.update(properties, children)
  }

  get parser() { return this.properties.parser }
  get src() { return `${this.file.path}.gif` }
}
